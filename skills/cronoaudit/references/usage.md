...
**The readings travel as data too.** `<name>-review-readings.json` carries every rule-built
sentence the report shows, in the report's language, with the language detection and its
confidence, so an assistant quotes the report's own reading instead of composing one.

**Every section carries a Reading** — sentences generated by rule from its own figures, in the
report's language. They say what the data supports and nothing more, and they are what lets
the report be handed over without a presenter. Separately, an **executive synthesis** can be
written by the person or the assistant running the skill and injected under its own label:

```
python3 scripts/inject_narrative.py <name>-review.html synthesis.txt --who "your name"
```

The report is rebuilt from its own embedded data, so the figures cannot drift from what the
text was written against, and the label says who wrote it and when.

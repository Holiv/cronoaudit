...
**Two layers of reading, so the report stands without a presenter.** Every section carries a
*Reading* block: sentences generated by rule from that section's own figures, in the report's
language, saying what the data supports and nothing more. And the report has a slot for an
**executive synthesis** written by whoever runs the skill with the figures in hand — labelled
with who wrote it and when, so prose is never mistaken for measurement.

**When you are the one running the skill, write that synthesis.** After `review.py` finishes,
read the JSON sidecars beside the HTML (`-review`, `-scurve`, `-productivity`, `-quality`,
`-forecast`, `-forensics`), and write four to six short paragraphs, in the schedule's language,
that a director could read in two minutes: whether the network holds and what that means for
every date below; where the weight is and whether the progress happened there; the Earned
Schedule reading with its limit stated; the two or three activities the forensics name; and
what has to happen in the next four weeks. Quote figures from the files, never from memory;
say what is `inferred` when it is; do not add anything the files do not support. Save it as
plain text with blank lines between paragraphs and inject it:

```
python3 scripts/inject_narrative.py <name>-review.html synthesis.txt --who "Claude, via the skill"
```

The report is regenerated from its own embedded data, so nothing is recomputed and the figures
cannot drift from what the synthesis was written against.
